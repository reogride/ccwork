...
노트에 부여하는 **북마크 상태**. `Note.isFavorited: boolean`으로 저장.

- 토글 단위: 노트 1개. 즐겨찾기 추가 = `isFavorited: true`, 해제 = `false`
- `PUT /notes/:id`로 DB에 즉시 반영 — 로컬 임시 상태 없음
- 신규 노트는 항상 `isFavorited: false`로 생성

## 즐겨찾기 탭 (Favorites Tab)

즐겨찾기된 노트만 모아 보는 **사이드바 탭**. "모든 노트" 탭과 전환.

- 빈 상태: "즐겨찾기한 노트가 없습니다" 메시지 표시
- 탭 내 NoteItem은 모든 노트 탭과 동일 (삭제 버튼 포함)
- ☆ 해제 시 해당 노트가 탭에서 사라지나 에디터는 유지
- "+ 새 노트" 클릭 시 "모든 노트" 탭으로 자동 전환
